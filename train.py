    ...
    with tf.Session(config=run_config) as sess:
        ...
        if train_dkt:
            ...
            for epoch in tqdm(range(args.num_epochs)):
                ...
                train_step = 0
                while not train_generator.end:  # an epoch
                    train_step += 1
                ...
                (
                    ...
                    f_score,
                    _,
                ) = precision_recall_fscore_support(targets, binary_preds)
                print(
                ...
                write_log(
                    args, model_dir, auc_value, accuracy, epoch, name="train_"
                )
                ...
                print(
                    "\nvalid auc={0}, accuracy={1}, precision={2}, recall={3}".format(
                        auc_value, accuracy, precision, recall
                    )
                )

                write_log(
                    args, model_dir, auc_value, accuracy, epoch, name="valid_"
                )
            ...
            print(model_dir)
            write_log(
                args, model_dir, auc_value, accuracy, epoch, name="test_"
            )
...
def save(global_step, sess, checkpoint_dir, saver):
    ...
    saver.save(
        sess, os.path.join(checkpoint_dir, model_name), global_step=global_step
    )
