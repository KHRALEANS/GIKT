...
def main():
    ...
    arg_parser.add_argument(
        "--dropout_keep_probs", nargs="?", default=[0.8, 0.8, 1]
    )
    arg_parser.add_argument("--aggregator", type=str, default="sum")
    arg_parser.add_argument("--model", type=str, default="hsei")
    ...
    arg_parser.add_argument(
        "--hist_neighbor_num", type=int, default=3
    )  # history neighbor num
    arg_parser.add_argument(
        "--next_neighbor_num", type=int, default=4
    )  # next neighbor num

    arg_parser.add_argument(
        "--att_bound", type=float, default=0.7
    )  # filtring irralate emb in topk selection
    arg_parser.add_argument(
        "--sim_emb", type=str, default="question_emb"
    )  # filtring irralate emb in topk selection
