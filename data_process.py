...
def data_process(args):
    ...
    (
        ...
        train_student_num,
        train_max_skill_id,
        train_max_question_id,
        feature_answer_id,
    ) = load_data(train_data_directory, args.field_size, args.max_step)
    (
        args.test_seqs,
        test_student_num,
        test_max_skill_id,
        test_max_question_id,
        _,
    ) = load_data(test_data_directory, args.field_size, args.max_step)
...
def build_adj_list(train_seqs, test_seqs, skill_matrix, qs_num):
    ...
    for seqs in [train_seqs, test_seqs]:
        for seq in seqs:
            ...
            for step in seq:
                adj_list[step[1]] = np.reshape(
                    np.argwhere(skill_matrix[step[0]] == 1), [-1]
                ).tolist()
                adj_num[step[1]] += 1
                for skill_index in np.reshape(
                    np.argwhere(skill_matrix[step[0]] == 1), [-1]
                ).tolist():
...
def extract_qs_relations(
    qs_list, s_num, qs_num, q_neighbor_size, s_neighbor_size
):
    question_neighbors = np.zeros(
        [qs_num, q_neighbor_size], dtype=np.int32
...
def sample_hist_neighbors(seqs_size, max_step, hist_num, skill_index):
    ...
    for i in range(seqs_size):
        ...
        for j in range(1, max_step):
            same_skill_index = [
                k for k in range(j) if seq_skill_index[k] == seq_skill_index[j]
            ]
...
def format_data(seqs, max_step, feature_size, hist_num):
    ...
    target_answers = pad_sequences(
        np.array([[j[-1] - feature_size for j in i[1:]] for i in seqs]),
        maxlen=max_step - 1,
        padding="post",
        value=0,
    )
    ...
    hist_neighbor_index = sample_hist_neighbors(
        len(seqs), max_step, hist_num, skills_index
    )  # [batch_size,max_step,M]
...
class DataGenerator(object):
    def __init__(
        self, seqs, max_step, batch_size, feature_size, hist_num
    ):  # feature_dkt
        np.random.seed(42)
        self.seqs = seqs
    ...
    def next_batch(self):
        ...
        batch_seqs = self.seqs[
            self.batch_i
            * self.batch_size : (self.batch_i + 1)
            * self.batch_size
        ]
        self.batch_i += 1
